-- Reviews are a revisitable writing surface, separate from dated watch notes.
-- A user can keep one review per film whether or not the film is in their diary.
create table public.reviews (
  ...
  constraint reviews_visibility_explicit check (
    visibility in ('private'::public.note_visibility, 'public'::public.note_visibility)
  )
);

create index reviews_user_updated_idx
...
-- The base table stays owner-only. Public profile reads use this narrow view,
-- which exposes only explicitly public reviews belonging to public profiles.
...
where profile.is_public
  and review.visibility = 'public'::public.note_visibility;
...
comment on table public.reviews is
  'One revisitable long-form film review per user and film, independent of dated watches.';
