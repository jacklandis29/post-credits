-- Keep diary edits and deletes owner-scoped and atomic. Direct table grants are
-- intentionally narrow; these functions also give deletion a safe place to
-- enforce ranking-session invariants.
...
begin
  ...
  if pg_catalog.cardinality(p_tags) > 10
    or (
      pg_catalog.cardinality(p_tags) > 0
      and pg_catalog.array_to_string(p_tags, ',')
        !~ '^([a-z0-9][a-z0-9-]{0,31})(,[a-z0-9][a-z0-9-]{0,31})*$'
    ) then
    raise exception 'Tags must contain at most ten normalized values'
      using errcode = '22023';
  end if;
...
begin
  ...
  if exists (
    ...
    raise exception 'Finish this entry''s ranking before deleting it'
      using errcode = '55000';
  end if;
  ...
  if p_remove_from_canon then
    ...
    where user_id = v_user_id
      ...
      and active_ranking_session_id is null;
  end if;
end;
$$;

revoke execute on function public.update_watch_entry(
  uuid, date, text, public.note_visibility, boolean, text[]
) from public, anon, authenticated, service_role;
revoke execute on function public.delete_watch_entry(uuid, boolean)
  from public, anon, authenticated, service_role;

grant execute on function public.update_watch_entry(
  uuid, date, text, public.note_visibility, boolean, text[]
) to authenticated;
grant execute on function public.delete_watch_entry(uuid, boolean)
  to authenticated;

comment on function public.update_watch_entry(
  uuid, date, text, public.note_visibility, boolean, text[]
) is 'Owner-scoped diary edit for date, note, visibility, spoiler flag, and tags.';
comment on function public.delete_watch_entry(uuid, boolean)
  is 'Owner-scoped atomic diary deletion with optional canon removal.';
