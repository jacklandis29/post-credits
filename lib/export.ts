...
export function exportUserData(
  ...
  if (format === "json") {
    ...
    download(`${slug}-${date}.json`, JSON.stringify(payload, null, 2), "application/json;charset=utf-8");
    return;
  }

  const header = [
    "record_type", "title", "year", "tmdb_id", "watched_on", "completion_status",
    "rewatch", "note", "review", "visibility", "rank", "score", "verdict", "added_at", "updated_at",
  ];
  const rows: unknown[][] = [
    ...state.diary.map((entry) => {
      const movie = movieById(entry.movieId);
      return ["watch", movie.title, movie.year, movie.id, entry.watchedOn, entry.completionStatus, entry.isRewatch, entry.note, "", entry.visibility, "", "", "", "", entry.createdAt];
    }),
    ...state.reviews.map((review) => {
      const movie = movieById(review.movieId);
      return ["review", movie.title, movie.year, movie.id, "", "", "", "", review.body, review.visibility, "", "", "", "", review.updatedAt];
    }),
    ...
    ...state.watchlist.map((item) => {
      ...
      return ["watchlist", movie.title, movie.year, movie.id, "", "", "", "", "", "", "", "", "", item.addedAt, ""];
    }),
  ];
