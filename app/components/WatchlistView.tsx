...
export function WatchlistView({
  ...
  const visibleItems = useMemo(() => {
    ...
    const next = items.filter((item) => movieById(item.movieId).title.toLowerCase().includes(normalized));
  ...
  function pickRandom() { if (visibleItems.length) onFilm(movieById(visibleItems[Math.floor(Math.random() * visibleItems.length)].movieId)); }
  return (
    <div className="page content-wrap watchlist-page">
      ...
      {items.length ? <div className="collection-tools watchlist-tools"><label className="search-field"><SearchIcon /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search your watchlist" aria-label="Search your watchlist" /></label><label className="compact-select"><span>Sort</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="added-desc">Recently added</option><option value="title">Title</option><option value="release-desc">Newest release</option><option value="release-asc">Oldest release</option><option value="runtime-asc">Shortest runtime</option><option value="runtime-desc">Longest runtime</option></select></label><button className="primary-action pick-random" type="button" onClick={pickRandom} disabled={!visibleItems.length}>Pick something for me</button></div> : null}
