    ...
    <div className="page content-wrap search-page">
      <div className="search-stage">
        <div className="search-title-block">
          ...
          {showFilters ? <p>Search a title, or browse by genre and decade when you don&rsquo;t have one in mind.</p> : null}
        </div>
        ...
        <section className="unified-search" aria-label="Search movies and people">
          ...
          {showFilters ? <div className="search-filter-row"><label className="compact-select"><span>Genre</span><select value={filters.genre} onChange={(event) => onFilters({ ...filters, genre: event.target.value })}><option value="all">All genres</option>{TMDB_GENRES.map((genre) => <option key={genre.id} value={genre.id}>{genre.name}</option>)}</select></label><label className="compact-select"><span>Decade</span><select value={filters.decade} onChange={(event) => onFilters({ ...filters, decade: event.target.value })}><option value="all">All decades</option>{DISCOVERY_DECADES.map((decade) => <option key={decade} value={decade}>{decade}s</option>)}</select></label><label className="compact-select"><span>Order</span><select value={filters.sort} onChange={(event) => onFilters({ ...filters, sort: event.target.value as DiscoveryFilters["sort"] })}><option value="popularity">Most popular</option><option value="newest">Newest releases</option></select></label></div> : null}
