...
export function DiaryView({
  ...
  const years = useMemo(
    () => [...new Set(entries.map((entry) => entry.watchedOn.slice(0, 4)))].sort((a, b) => b.localeCompare(a)),
    [entries],
  );
  ...
  const visibleEntries = useMemo(() => {
    const filtered = entries.filter((entry) =>
      (year === "all" || entry.watchedOn.startsWith(year)) &&
    ...
    return next;
  }, [entries, shuffleSeed, sort, tag, year]);
  ...
  return (
    <div className="page content-wrap diary-page">
      ...
      {entries.length ? (
        <div className="collection-tools diary-tools">
          <div className="year-jump" aria-label="Jump to diary year">
            <button className={year === "all" ? "active" : ""} type="button" onClick={() => setYear("all")}>All</button>
            {years.map((item) => <button className={year === item ? "active" : ""} type="button" key={item} onClick={() => setYear(item)}>{item}</button>)}
          </div>
          <label className="compact-select"><span>Sort</span><select value={sort} onChange={(event) => setSort(event.target.value)}>
            <option value="watched-desc">Newest watched</option>
          ...
          {tags.length ? <label className="compact-select"><span>Tag</span><select value={tag} onChange={(event) => setTag(event.target.value)}><option value="all">All tags</option>{tags.map((item) => <option key={item} value={item}>#{item}</option>)}</select></label> : null}
            ...
            {entries.map((entry) => {
              ...
              return (
                <article className="diary-card" key={entry.id} style={filmStyle(movie)}>
                  <button className="diary-poster" onClick={() => onFilm(movie)} aria-label={`${movie.title} details`}>
                    ...
                    <span className="diary-overlay">
                      ...
                      {entry.tags?.length ? <span className="diary-entry-tags">{entry.tags.slice(0, 2).map((item) => <small key={item}>#{item}</small>)}</span> : null}
                  ...
                  <button className="diary-edit" type="button" onClick={() => setEditingEntry(entry)} aria-label={`Edit ${movie.title} diary entry`}>Edit</button>
                </article>
              );
            })}
          </div>
      ...
      {editingEntry ? (
        <DiaryEntrySheet
          entry={editingEntry}
          movie={movieById(editingEntry.movieId)}
          isOnlyWatch={state.diary.filter((entry) => entry.movieId === editingEntry.movieId).length === 1}
          isRanked={state.ranked.some((row) => row.movieId === editingEntry.movieId)}
          busy={busy}
