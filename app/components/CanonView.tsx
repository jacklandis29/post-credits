...
export function CanonView({
  ...
  const watchedYears = useMemo(
    () => [...new Set(diary.map((entry) => entry.watchedOn.slice(0, 4)))].sort((a, b) => b.localeCompare(a)),
    [diary],
  );
  const decades = useMemo(
    () => [...new Set(rows.map((row) => Math.floor(row.movie.year / 10) * 10))].sort((a, b) => b - a),
    [rows],
  );
  const genres = useMemo(
    () => [...new Set(rows.flatMap((row) => row.movie.genres))].sort((a, b) => a.localeCompare(b)),
    [rows],
  );
  const visibleRows = useMemo(() => {
    ...
    const next = rows.filter((row) => {
      ...
      return (
        (verdict === "all" || row.ranked.verdict === verdict) &&
        (watchedYear === "all" || watches.some((entry) => entry.watchedOn.startsWith(watchedYear))) &&
        (releaseDecade === "all" || Math.floor(row.movie.year / 10) * 10 === Number(releaseDecade)) &&
        (genre === "all" || row.movie.genres.includes(genre)) &&
        (rewatched === "all" || (rewatched === "yes" ? watches.some((entry) => entry.isRewatch) : !watches.some((entry) => entry.isRewatch))) &&
        (!normalizedQuery || row.movie.title.toLowerCase().includes(normalizedQuery) || row.movie.director.toLowerCase().includes(normalizedQuery))
      );
    });
  ...
  return (
    <div className="page content-wrap canon-page">
      ...
      <div className="canon-filter-grid" aria-label="Ranking filters and sort">
        <label className="compact-select"><span>Watched</span><select value={watchedYear} onChange={(event) => setWatchedYear(event.target.value)}><option value="all">Any year</option>{watchedYears.map((year) => <option key={year} value={year}>{year}</option>)}</select></label>
        <label className="compact-select"><span>Released</span><select value={releaseDecade} onChange={(event) => setReleaseDecade(event.target.value)}><option value="all">Any decade</option>{decades.map((decade) => <option key={decade} value={decade}>{decade}s</option>)}</select></label>
        <label className="compact-select"><span>Genre</span><select value={genre} onChange={(event) => setGenre(event.target.value)}><option value="all">Any genre</option>{genres.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        <label className="compact-select"><span>Rewatched</span><select value={rewatched} onChange={(event) => setRewatched(event.target.value)}><option value="all">Either</option><option value="yes">Rewatched</option><option value="no">First watches</option></select></label>
        <label className="compact-select"><span>Sort</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="rank">Canon rank</option><option value="release-desc">Newest release</option><option value="release-asc">Oldest release</option><option value="runtime-asc">Shortest runtime</option><option value="runtime-desc">Longest runtime</option><option value="shuffle">Shuffle</option></select></label>
        {sort === "shuffle" ? <button className="secondary-action small" type="button" onClick={() => setShuffleSeed((seed) => seed + 1)}>Shuffle again</button> : null}
      </div>
      <p className="collection-result-count" aria-live="polite">Showing {visibleRows.length} of {total} films</p>
